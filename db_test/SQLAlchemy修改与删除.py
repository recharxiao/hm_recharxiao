...
from sqlalchemy import create_engine, Integer, String, DateTime, delete, update
  # 从sqlalchemy核心模块导入: create_engine(创建引擎), Integer(整型), String(字符串型), DateTime(日期时间型)
...
if __name__ == '__main__':  # 程序入口: 仅当直接运行该脚本时执行以下代码, 被导入时不执行
    ...
    #                 .where(AiMessage.id ==9)
    #                 .values(content = '我是张无忌',create_time = datetime.now())
    #                 )
    # session.commit()
    #
    # ai_message = session.get(AiMessage,13)
    # print(ai_message.content)
    #
    # ai_message.content = '我是张无忌'
    # ai_message.session_id = 2
    #
    # session.commit()

    # result = session.execute(delete(AiMessage).where(AiMessage.id == 13))
    # session.commit()

    session.execute(delete(AiMessage).where(AiMessage.id.in_([1,2,3])))
    session.commit()