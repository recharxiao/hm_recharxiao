...
from sqlalchemy import create_engine, Integer, String, DateTime  # 从sqlalchemy核心模块导入: create_engine(创建引擎), Integer(整型), String(字符串型), DateTime(日期时间型)
...
if __name__ == '__main__':  # 程序入口: 仅当直接运行该脚本时执行以下代码, 被导入时不执行
    ...
    # 3.2 执行操作
    with Session(engine) as session:  # 使用上下文管理器创建会话, 自动管理会话的生命周期(退出with块时自动关闭连接)
        session.add(AiMessage(session_id=1, role="assistant", content="你笑什么呢~", create_time=datetime.now()))  # 创建一条AiMessage记录(会话1, assistant角色), 并添加到会话的待提交列表中
        session.add(AiMessage(session_id=1, role="user", content="就是好笑呀", create_time=datetime.now()))  # 再创建一条AiMessage记录(会话1, user角色), 添加到会话的待提交列表中
        session.commit()  # 提交事务, 将上述两条记录正式写入数据库, 若失败则自动回滚
